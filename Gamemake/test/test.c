...
int Gameover(void)
{
    ...
    // 플레이어 이름과 점수(예시를 위해 임시 100점 설정, 실제 연산 변수가 있다면 변경 가능) 및 팀 명단을 넣습니다.
    const char* credits[] = {
        "======================================",
        "              GAME OVER               ",
        "======================================",
        ...
        "",
        "======================================",
        "   Press Backspace to Return Menu...  ",
        "======================================"
    };
    ...
    // SCREEN_HEIGHT는 30으로 정의되어 있으므로 i는 30부터 시작합니다.
    for (int i = SCREEN_HEIGHT; i >= -num_lines; i--)
    {
        // 화면을 무식하게 system("cls")로 지우면 깜빡임이 심하므로, 
        // ANSI escape code "\x1b[2J"를 사용해 화면을 깔끔하게 지우고 홈 위치로 복귀합니다.
        printf("\x1b[2J\x1b[H");
        set_color(FONT_COLOR_WHITE);

        // 현재 프레임에서 화면에 보여야 하는 줄들만 계산해서 출력합니다.
        for (int j = 0; j < num_lines; j++)
        {
            ...
            if (line_y >= 0 && line_y < SCREEN_HEIGHT)
            {
                ...
                // 영어/한글 혼용 폭 계산 함수인 calculate_visual_length를 활용합니다.
                ...
                // 플레이어 이름 텍스트 줄(9번째 줄 인덱스)은 %s 서식 지정자로 처리하여 전역변수를 출력합니다.
                if (j == 9)
                {
        ...
        // 커서를 화면 오른쪽 아래 구석으로 대피시켜 깜빡임을 방지합니다.
        move_cursor(SCREEN_WIDTH - 1, SCREEN_HEIGHT - 1);

        // 올라가는 속도를 조절하는 딜레이 타임 (밀리초 단위, 숫자가 작을수록 빨라짐)
        Sleep(150);
    }
    ...
    exit(0); // 10초간의 긴 여운하고 작별 식순이 전면 완전히 종료되었으므로, C 표준 라이브러리 시스템 최고 핵심 명령을 하달하여 구동 중이던 콘솔창 게임 프로그램 프로세스 자원 자체를 운영체제 메모리에서 통째로 전면 강제 클로즈 즉각 완전 종료 폐쇄 아웃시킵
