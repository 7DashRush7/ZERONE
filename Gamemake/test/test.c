...
void ShowLogo(void)
{
    ...
    move_cursor(logo_start_x + 3, logo_start_y + logo_height + 2); // 안내 문구를 출력하기 위해 적절한 위치로 커서를 옮깁니다.
    set_color(FONT_COLOR_WHITE); // 안내문 글자색을 흰색으로 변경합니다.
    printf("전체화면으로 바꾼 뒤 Enter키를 누르면 시작합니다."); // 콘솔 안내 멘트를 노출합니다.
...
int Gameover(void)
{
    int y = 30;  // 화면의 맨 밑바닥 경계선 최하단 줄 위치(30번째 세로 줄)를 첫 번째 엔딩 크레딧 자막 라인의 시동 연출 세로축 시작점으로 초기화 세팅합니다.
    int yy = 30; // 화면의 맨 밑바닥 경계선 최하단 줄 위치를 연속해 뒤따라 올라올 두 번째 엔딩 크레딧 자막 라인의 시동 연출 세로축 시작점으로 대칭 초기화 세팅합니다.
    int While = 1; // 첫 번째 엔딩 자막 줄을 위로 슬라이딩 업 스크롤링 기동시키는 프레임 애니메이션 제어 루프의 가동 참(1) 스위치입니다.
    int Thile = 1; // 두 번째 엔딩 자막 줄을 입체감 있게 가속 추격 슬라이딩 업 스크롤링 기동시키는 프레임 애니메이션 제어 루프의 가동 참(1) 스위치입니다.

    system("cls"); // 작별 식순 연출을 경건하고 엄숙하게 전개하기 전 화면 내벽을 올 클리어 청소 처리합니다.

    while (While) // 첫 번째 작별 자막 문구를 밑바닥에서부터 천장까지 정밀 무빙업 슬라이딩 스크롤시키는 기동 루프 스레드입니다.
    {
        if (y != 1) // 첫 번째 자막 선두의 세로 위치 좌표가 콘솔창 꼭대기 상단 한계선인 1번째 줄 세로 위치 좌표에 완벽히 도달 안 한 도중 상황 동안 계속 작동합니다.
        {
            system("cls"); // 한 프레임 한 프레임 문장을 위로 갱신해 다시 그릴 때마다 이전 프레임 위치에 찍혀 잔상으로 오염된 모니터 글자 자국들을 실시간으로 완전 소독 세정 밀어버립니다.
            move_cursor(42, y); printf("여기에 마무리 되는거 추가로 넣고 꺼지게 하기"); // 매 프레임 실시간 위로 갱신 이동 처리되는 동적 가변 Y좌표선 가로 정렬선 자리에 맞춰 준비된 종막 마감 서사 자막을 드로우 출력합니다.
            y--; // 세로 높이 좌표 수치를 1씩 마이너스 차감 삭감합니다. (컴퓨터 화면 좌표계 특성상 Y값이 줄어들수록 글자는 모니터 위쪽 하늘 방향으로 한 칸씩 상승 이동 이동하게 됩니다.)

            Sleep(100); // 눈 깜짝할 사이에 문장이 천장으로 순간이동 폭주 비행하지 않고, 인간의 육안으로 스크롤 무빙 무브먼트를 부드럽게 시각 인지할 수 있도록 매 프레임 드로우 단계마다 0.1초(100ms)의 정밀 프레임 타임 슬라이스 동결 딜레이를 부여해 제어합니다.

            if (y == 1) // 첫 번째 엔딩 자막 선두가 기어코 목표 꼭대기 타깃 고지인 최상층 세로 1번 줄 자리에 완벽히 안착 터치 성공한 조건 성립 타이밍 상황입니다.
            {
                While = 0; // 목적지 도달 완료를 선언하며 첫 번째 무빙 스크롤 애니메이션 루프 제어 스위치를 거짓(0)으로 꺼트려 루프를 이탈 탈출합니다.
            }
        }
    }

    while (Thile) // 선두 문장이 꼭대기에 자리를 잡고 대기 타는 동안, 연이어 바닥에서부터 입체감 있게 추격 업로드 스크롤링되어 올라오는 두 번째 작별 자막 연출용 코어 무한 루프 스레드입니다.
    {
        if (yy != 3) // 두 번째 후발대 자막의 세로 위치 좌표가 선두 문장의 바로 아래 공간 2칸 밑 마진선 자리인 세로 3번째 줄 목표 좌표 자리에 아직 도달하지 못한 상황 동안 무한 가동합니다.
        {
            system("cls"); // 가속 프레임 드로우 리프레시를 위해 매 프레임 콘솔 터미널 텍스트 잔상 화면을 실시간 전면 세정 처리합니다.
            move_cursor(42, y); printf("여기에 마무리 되는거 추가로 넣고 꺼지게 하기"); // 이미 아까 꼭대기 최종 타깃 목표 고지(y=1)에 완벽 무사 안착 고정되어 멈춰 서 있는 첫 번째 선두 마감 자막 라인을 매 프레임 화면 소실 없이 유지되도록 상단 고정 드로우를 계속 수행해 줍니다.

            move_cursor(42, yy); printf("여기에 마무리 되는거 추가로 넣고 꺼지게 하기"); // 선두 문장을 향해 실시간 바닥에서부터 0.1초마다 한 칸씩 역동적으로 상공 스크롤 무빙 기동하며 치고 올라오는 후발대 두 번째 마감 자막 라인을 가변 세로축 좌표 yy 위치에 맞춰 대칭 드로우 출력합니다.
            yy--; // 후발대 세로 높이 수치 좌표를 1씩 마이너스 차감하여 하늘 위 방향으로 맹렬히 상승 기동시킵니다.

            Sleep(100); // 부드러운 스크롤 스무스 입체감 연출 화면 주사율 싱크 보존을 위해 0.1초 정밀 프레임 타임 슬라이스 동결 딜레이 록을 걸어 제어합니다.

            if (yy == 3) // 후발대 추격 자막 문장이 기어코 목표 약속의 안착 고지선인 상단 세로 3번 줄 자리에 완벽 도달 도킹에 성공한 감격의 조건 성립 완료 상황입니다.
            {
                Thile = 0; // 입체 스크롤 애니메이션 모듈 임무 완수를 선언하며 두 번째 스위치를 거짓(0)으로 꺾어 루프창을 최종 이탈 탈출 마감합니다.
            }
        }
    }
