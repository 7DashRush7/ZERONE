...
char playerName[50] = "Player"; // [추가됨] 플레이어의 닉네임을 저장할 전역 변수
...
Choice choices[] =
{
    ...
    { { "   +++   ", "  +HP+  ", "   +++   ", "         ", "         ", "         " }, "약국에서 진통제를 복용한다.", -15, -10 },// hp+선택지
...
int RenderTitle(void)
{
    ...
    set_color(FONT_COLOR_GREEN); // 눈에 띄게 초록색으로 설정
    move_cursor(10, 3);
    printf("환영합니다, [%s] 님!", playerName);

    set_color(BG_COLOR_BRIGHTMAGENTA); // 다시 타이틀 배경색 복구
...
int Gamestart(void)
{
    ...
    while (hp > 0)
    {
        ...
        // 상단 중앙 UI: 현재 HP (간격을 위해 기존 x좌표 40에서 45로 이동)
        set_color(FONT_COLOR_RED);
        move_cursor(45, 2);
        printf("HP : %d", hp);
        ...
        // 상단 우측 UI: 현재 SCORE
        set_color(FONT_COLOR_WHITE);
        move_cursor(75, 2); // 간격을 위해 기존 x좌표 70에서 75로 이동
        ...
        hp -= damage; // 플레이어의 현재 체력에서 계산된 데미지를 차감, damage가 음수면 HP가 회복됨

        if (hp > 100)
        {
            hp = 100; // HP가 100을 넘지 않게 제한
        }

        score += 1; // 라운드를 한 턴 넘겼으므로 점수 1 증가
        ...
        if (damage < 0)
        {
            printf("선택 완료! HP가 %d 회복되었습니다.", -damage); // 회복 결과 출력
        }
        else
        {
            printf("선택 완료! HP가 %d 감소했습니다.", damage); // 데미지 결과 출력
        }
        Sleep(2000); // 사용자가 읽을 수 있도록 2초(2000ms) 대기 후 다음 라운드로 루프 반복

        // 대기 시간 동안 사용자가 연타해서 쌓인 키 입력 버퍼 비우기
        // _kbhit()은 버퍼에 입력된 키가 있으면 1, 없으면 0을 반환합니다.
        // 키가 남아있는 동안 _getch()로 허공에 읽어들여서 버퍼를 완전히 비웁니다.
        while (_kbhit())
        {
            _getch();
        }
...
int main(void)
{
    ...
    system("cls"); // 로고 화면 지우기
    set_color(FONT_COLOR_WHITE);
    move_cursor(45, 14); // 화면 중앙쯤으로 커서 이동
    printf("플레이어의 닉네임을 입력하세요: ");

    set_color(FONT_COLOR_YELLOW); // 입력할 텍스트 색상을 노란색으로 강조
    scanf("%49s", playerName);    // 닉네임 입력받기 (배열 크기 초과 방지 위해 %49s 사용)

    // 입력 버퍼 비우기 (엔터키 등 찌꺼기가 남아 이후 _getch()가 오작동하는 것을 방지)
    while (getchar() != '\n');

    set_color(FONT_COLOR_WHITE); // 색상 원상복구
    system("cls"); // 입력이 끝나면 다시 화면을 깨끗하게 지움
